...
def ft_tqdm(lst: range) -> any:
    '''Display a progress bar for the given list'''
    total = len(lst)
    terminal_size = shutil.get_terminal_size()
    length = (terminal_size.columns) / 1.3
    start_time = time.time()
    progress_format = ("{percentage:3.0f}%|{bar}| {item:3}/{total}"
                       " [{etime}<{rtime}, {iter:3.2f}it/s]    ")
    ...
    for i, item in enumerate(lst):
        ...
        sys.stdout.write("\r" + progress_format.format(total=total,
                                                       item=item + 1,
                                                       bar=bar,
